...
- The fish add-to-a-list variant uses a 640px content column with 40px between the main heading section and the questions stack, 56px between fish question blocks, 24px between each question text group and its control, no extra default H2 margin inside the heading section, full-width 640px by 48px inputs/selects, 24px between the length/weight label and input, and a link-style length/weight switch. The first state uses the standard primary `Save and continue` button, not a separate add button. The added-fish state uses `You have added X type(s) of fish`, Change and Remove actions, and `Do you need to add another type of fish?`.
